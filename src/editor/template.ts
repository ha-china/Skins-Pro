    ...
    <div class="sp-wrap">
        ...
        <div class="sp-row" style="grid-template-columns:1fr 1fr;gap:12px">
          <label class="sp-field">
            ...
            <div style="display:flex;flex-wrap:nowrap;align-items:center;gap:8px">
              ...
              ${c.background_image ? `<img class="bg-preview" src="${escapeHtml(c.background_image)}"><button class="sp-del" data-bg-clear>✕</button>` : ''}
            </div>
          </label>
      ...
      <div class="sp-row">
        <div class="sp-card">
          ...
          ${listPicker(loc('devices'), 'home_selection.devices', hs.devices || [], HOME_DEVICE_DOMAINS, hl.devices ?? DEFAULT_CONFIG.home_limits!.devices!)}
        </div>
        <div class="sp-card">
          <h3>${loc('editorHomeRooms')}</h3>
          ${areaPicker(data.areas, data.areasLoaded, hs.rooms || [], hl.rooms ?? DEFAULT_CONFIG.home_limits!.rooms!, data.language)}
        </div>
      </div>
      ...
      <div class="sp-row">
        <div class="sp-card">
          ...
          ${listPicker(loc('scenes'), 'home_selection.scenes', hs.scenes || [], ['scene'], hl.scenes ?? DEFAULT_CONFIG.home_limits!.scenes!)}
        </div>
        <div class="sp-card">
          <h3>${loc('editorHomeEnv')}</h3>
          ${listPicker(loc('environment'), 'home_selection.environment', hs.environment || [], ['sensor'], hl.environment ?? DEFAULT_CONFIG.home_limits!.environment!)}
        </div>
      </div>
